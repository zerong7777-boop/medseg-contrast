...
class group_aggregation_bridge(nn.Module):
    ...
    def forward(self, xh, xl, mask=None):
        ...
        # 3) 可选的 mask：缺省时用全 1；尺寸不匹配则插值
        if mask is None:
            B, _, H, W = xl.shape
            mask = torch.ones(B, 1, H, W, device=xl.device, dtype=xl.dtype)
        elif mask.shape[2:] != xl.shape[2:]:
        ...
        # 5) 拼接 + 尾部 1x1
        x = torch.cat((x0, x1, x2, x3), dim=1)  # 通道数 = 2*dim_xl + 4
        x = self.tail_conv(x)
        return x
...
if __name__ == '__main__':
    M = EGEUNet(num_classes=2, input_channels=3, gt_ds=False)
    print(M(torch.randn(3,3,256,256)).shape)