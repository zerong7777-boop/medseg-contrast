import os
import cv2
import numpy as np
import nibabel as nib


def convert_grayscale_to_class(image):
    """
    将灰度值映射为类值。
    """
    # 创建一个新的图像，初始化为0（背景类）
    new_image = np.zeros_like(image)

    # 映射灰度值到类

#COVID19_CT
    new_image[image == 255] = 1
    new_image[image == 0] = 0
    # new_image[image == 12] = 1
    # new_image[image == 17] = 1
    # new_image[image == 21] = 1




    return new_image


def process_png_images(input_folder, output_folder):
    """
    处理PNG图像，将灰度值映射为类别并保存。
    """
    # 如果输出文件夹不存在，创建它
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)

    # 遍历文件夹中的所有PNG图像
    for filename in os.listdir(input_folder):
        if filename.endswith(".png"):
            # 读取图像
            image_path = os.path.join(input_folder, filename)
            image = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)

            # 将灰度值转换为类
            converted_image = convert_grayscale_to_class(image)

            # 保存转换后的图像
            output_path = os.path.join(output_folder, filename)
            cv2.imwrite(output_path, converted_image)
            print(f"Processed and saved PNG: {output_path}")


def process_nii_images(input_folder, output_folder):
    """
    处理NIfTI图像，将灰度值映射为类别并保存。
    """
    # 如果输出文件夹不存在，创建它
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)

    # 遍历文件夹中的所有NIfTI图像
    for filename in os.listdir(input_folder):
        if filename.endswith(".nii.gz"):
            # 读取NIfTI图像
            image_path = os.path.join(input_folder, filename)
            nii_image = nib.load(image_path)
            image_data = nii_image.get_fdata().astype(np.uint8)  # 读取数据并转换为uint8

            # 将灰度值转换为类
            converted_data = convert_grayscale_to_class(image_data)

            # 保存转换后的NIfTI图像
            output_path = os.path.join(output_folder, filename)
            converted_nii = nib.Nifti1Image(converted_data, nii_image.affine, nii_image.header)
            nib.save(converted_nii, output_path)
            print(f"Processed and saved NIfTI: {output_path}")


def process_images_in_folder(input_folder, output_folder):
    """
    处理文件夹中的所有图像，包括PNG和NIfTI格式。
    """
    process_png_images(input_folder, output_folder)
    process_nii_images(input_folder, output_folder)


if __name__ == "__main__":
    # 输入文件夹路径
    input_folder = '/home/jgzn/PycharmProjects/RZ/danzi/seg/datasets/ISIC/train/label_ori'

    # 输出文件夹路径
    output_folder = '/home/jgzn/PycharmProjects/RZ/danzi/seg/datasets/ISIC/train/label'

    # 处理图像
    process_images_in_folder(input_folder, output_folder)
